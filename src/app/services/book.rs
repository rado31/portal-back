...
pub async fn all_for_admin(req: Request<State>) -> Result<Response> {
    ...
    match repositories::book::all_for_admin(pool).await {
        Ok(musics) => {
        ...
        Err(error) => {
            log::error!("Get books for admin: {error}");
            Ok(Response::new(500))
        }
    }
}
...
pub async fn upload(req: Request<State>) -> Result<Response> {
    ...
    if !json_file.book_id_exists(book_id as i32) {
        json_file.books.push(book_id as i32);
    };
...
pub async fn delete(req: Request<State>) -> Result<Response> {
    ...
    match repositories::book::delete(pool, book_id as i32).await {
        Ok(rows_affected) => {
            ...
            json_file.deleted.books.push(book_id as i32);
            json_file.remove_book(book_id as i32);
